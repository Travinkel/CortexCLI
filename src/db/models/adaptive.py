...
class LearnerMasteryState(Base):
    ...
    unlock_reason: Mapped[str | None] = mapped_column(Text)  # 'mastery', 'waiver', 'prerequisite_met', 'no_prerequisites'
...
class LearningPathSession(Base):
    ...
    session_mode: Mapped[str] = mapped_column(Text, default="adaptive")  # 'adaptive', 'review', 'quiz', 'remediation'

    # Session state
    status: Mapped[str] = mapped_column(Text, default="active")  # 'active', 'paused', 'completed', 'abandoned'
    ...
    target_cluster: Mapped["CleanConceptCluster | None"] = relationship(foreign_keys=[target_cluster_id])
    ...
    responses: Mapped[list["SessionAtomResponse"]] = relationship(back_populates="session", cascade="all, delete-orphan")
...
class SessionAtomResponse(Base):
    ...
    id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    session_id: Mapped[UUID] = mapped_column(ForeignKey("learning_path_sessions.id", ondelete="CASCADE"), nullable=False)
    ...
    hint_level_used: Mapped[int] = mapped_column(Integer, default=0)  # 0=none, 1=hint1, 2=hint2, 3=answer
...
class AtomTypeSuitability(Base):
    ...
    id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    atom_id: Mapped[UUID] = mapped_column(ForeignKey("learning_atoms.id", ondelete="CASCADE"), nullable=False, unique=True)
    ...
    knowledge_signal: Mapped[Decimal | None] = mapped_column(Numeric(4, 3))  # Primary: knowledge type alignment
    structure_signal: Mapped[Decimal | None] = mapped_column(Numeric(4, 3))  # Secondary: content structure
    ...
    computation_method: Mapped[str] = mapped_column(Text, default="rule_based")  # 'rule_based', 'ai_scored', 'hybrid'

    # Relationships
    atom: Mapped["CleanAtom"] = relationship(back_populates="suitability")

    __table_args__ = (
        Index("idx_suitability_mismatch", "type_mismatch"),
    )
...
class RemediationEvent(Base):
    ...
    id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    session_id: Mapped[UUID | None] = mapped_column(ForeignKey("learning_path_sessions.id", ondelete="SET NULL"))
    learner_id: Mapped[str] = mapped_column(Text, nullable=False, index=True)
    ...
    trigger_type: Mapped[str] = mapped_column(Text, nullable=False)  # 'incorrect_answer', 'low_confidence', 'prerequisite_gap', 'manual'
    ...
    remediation_successful: Mapped[bool | None] = mapped_column(Boolean)  # Did mastery reach threshold?
