...
--   - HNSW indexing for fast approximate nearest neighbor queries
--   - 384-dimensional embeddings from sentence-transformers (all-MiniLM-L6-v2)
--
-- Data Flow:
--   clean_atoms → Embedding Generation → Similarity Analysis → Duplicate Detection
...
-- ========================================
-- INDEXES: HNSW for Approximate Nearest Neighbor
-- ========================================

-- HNSW index on clean_atoms for fast similarity search
-- Parameters: m=16 (connections per layer), ef_construction=64 (build quality)
-- Provides ~95% recall with 10x faster queries than brute force
CREATE INDEX IF NOT EXISTS idx_clean_atoms_embedding_hnsw
ON clean_atoms USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64);

-- HNSW index on clean_concepts for prerequisite inference
CREATE INDEX IF NOT EXISTS idx_clean_concepts_embedding_hnsw
ON clean_concepts USING hnsw (embedding vector_cosine_ops)
WITH (m = 16, ef_construction = 64);

-- Standard index for filtering atoms without embeddings
...
-- Function: Find similar atoms using cosine similarity
CREATE OR REPLACE FUNCTION find_similar_atoms(
...
AS $$
    SELECT
        ...
        a.card_id,
        a.front,
        (1 - (a.embedding <=> target_embedding))::DECIMAL as similarity
    FROM clean_atoms a
...
-- Function: Get semantic duplicate pairs above threshold
CREATE OR REPLACE FUNCTION find_semantic_duplicate_pairs(
...
AS $$
    SELECT
        ...
        a1.front as front_1,
        a2.front as front_2,
        (1 - (a1.embedding <=> a2.embedding))::DECIMAL as similarity
    FROM clean_atoms a1
    CROSS JOIN clean_atoms a2
    WHERE a1.id < a2.id  -- Prevent duplicates and self-comparison
      AND a1.embedding IS NOT NULL
      AND a2.embedding IS NOT NULL
      AND (1 - (a1.embedding <=> a2.embedding)) > similarity_threshold
...
COMMENT ON COLUMN knowledge_clusters.centroid IS '384-dim vector representing cluster center';
...
COMMENT ON COLUMN clean_atoms.embedding IS '384-dim semantic embedding from sentence-transformers (all-MiniLM-L6-v2)';
COMMENT ON COLUMN clean_atoms.embedding_model IS 'Model used to generate embedding (for reproducibility)';

COMMENT ON FUNCTION find_similar_atoms IS 'Find atoms similar to a target embedding using cosine similarity';
COMMENT ON FUNCTION find_semantic_duplicate_pairs IS 'Find all pairs of atoms above similarity threshold (for duplicate detection)';
COMMENT ON FUNCTION invalidate_embedding_on_content_change IS 'Auto-clears embedding when front/back content is modified';
