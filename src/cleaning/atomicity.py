...
MALFORMED_QUESTION_PATTERNS = [
    ...
    re.compile(r"In\s+networking,\s+what\s+\w+\s+The\s+", re.IGNORECASE),  # "In networking, what internet The internet"
    re.compile(r"what\s+networking\s+term\s+describes:\s*are\b", re.IGNORECASE),  # "describes: are a type"
    ...
    re.compile(r"networking\s+term\s+describes:\s*[A-Z][a-z]+\s+layer", re.IGNORECASE),  # "describes: Application layer" (just OSI layer name)
    re.compile(r"what\s+\w+\s+If\s+", re.IGNORECASE),  # "what one If one path fails"
    re.compile(r"In\s+networking,\s+what\s+\w+\s+If\s+", re.IGNORECASE),  # "In networking, what X If"
]
INCOHERENT_PATTERNS = [
...
class CardQualityAnalyzer:
    ...
    def analyze(
        ...
        if self._detect_malformed_question(front):
            ...
            recommendations.append(
                "Question appears malformed or truncated. "
        ...
        if self._detect_too_short(front, back):
            ...
            recommendations.append(
                "Content too brief to be meaningful for learning. "
        ...
        if self._detect_generic_question(front):
            ...
            recommendations.append(
                "Question is too generic/vague. "
                "Add context about what aspect is being asked."
            )
        ...
        # Determine flags
        is_atomic = (
            QualityIssue.ENUMERATION_DETECTED not in issues and QualityIssue.MULTIPLE_FACTS not in issues
        )
    ...
    def _contains_enumeration(self, text: str) -> bool:
        ...
        for line in lines:
            ...
            if any(line.startswith(prefix) for prefix in LIST_PREFIXES):
                list_line_count += 1
            # Check numbered markers (1. 2. 3. or 1) 2) 3))
            elif NUMBERED_PATTERN.match(line):
                list_line_count += 1
            # Check letter markers (a. b. c. or a) b) c))
            elif LETTER_PATTERN.match(line):
                list_line_count += 1
    ...
    def _detect_multi_subquestion(self, text: str) -> bool:
        ...
        if any(pattern in lower for pattern in compound_patterns):
            return True
    ...
    def _detect_malformed_question(self, text: str) -> bool:
        ...
        for pattern in MALFORMED_QUESTION_PATTERNS:
