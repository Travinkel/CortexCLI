...
# Optional injection for diagram-as-code when visuals are implied
VISUAL_PROMPT_INJECTION = """VISUAL REQUIREMENT (Diagram-as-Code):
- IF the concept is a Topology, Process/Sequence, or Hierarchy, you MUST include a Mermaid.js diagram in a markdown code block.
- Use graph TD for topologies/trees and sequenceDiagram for protocol flows/handshakes.
- Example:
```mermaid
graph TD
  Internet -->|WAN| Router
...
DO NOT include conversational text ("Here is the JSON").
...
def get_prompt(
    ...
    def needs_visual_injection(text: str) -> bool:
        lowered = text.lower()
        keywords = [
            "topology",
            "diagram",
    ...
    # Inject diagram-as-code guidance when the content suggests a visual is needed
    if atom_type.lower() in {"flashcard", "concept", "cloze"} and needs_visual_injection(content):
        template = VISUAL_PROMPT_INJECTION + "\n" + template
