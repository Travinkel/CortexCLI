...
# Prompt for generating personalized study notes
STUDY_NOTES_PROMPT = """You are an expert CCNA tutor creating personalized study notes.

Your task is to create concise, memorable study notes that help the learner master challenging networking concepts.

The learner's context will be provided including:
- Topics/sections they struggled with
- Specific error patterns (misconceptions, confusion)
- Current cognitive state (encoding strength, fatigue level)

GUIDELINES:
1. Address specific misconceptions and gaps identified
2. Use analogies and mental models for better retention
3. Highlight key distinctions that commonly cause confusion
4. Include mnemonic devices where helpful
5. Format for quick scanning (bullets, bold key terms)
...
### Core Concept
[Clear 1-2 sentence explanation of the key idea]

### What You Got Wrong
- [Misconception] → Actually: [Correct understanding]

### Memory Anchors
- [Mnemonic, analogy, or mental model to aid recall]
...
IMPORTANT:
- Be specific to networking/CCNA content
- Don't be generic - address the EXACT errors shown
- Use technical terms correctly
- Include command syntax where relevant
"""
...
Return JSON array only, no markdown code blocks."""
    ...
    def generate_study_notes(
        ...
        section_id: str,
        section_title: str = "",
        struggle_data: dict | None = None,
        error_patterns: list[str] | None = None,
        cognitive_state: dict | None = None,
        depth: str = "standard",
    ) -> str:
        """
        Generate personalized study notes for remediation.

        Args:
            section_content: The source content for the section
            section_id: CCNA section ID (e.g., "5.2.1")
            section_title: Title of the section
            struggle_data: Dict with struggle info (severity, weight, failure_modes)
            error_patterns: List of specific error patterns/misconceptions observed
            cognitive_state: Dict with cognitive metrics (encoding, fatigue, etc.)
            depth: "brief", "standard", or "comprehensive" - affects note length

        Returns:
            Markdown-formatted study notes string
        """
        ...
        # Struggle context
        if struggle_data:
            severity = struggle_data.get("severity", "unknown")
            weight = struggle_data.get("weight", 0.5)
            failure_modes = struggle_data.get("failure_modes", [])
            context_parts.append(f"STRUGGLE SEVERITY: {severity} (weight: {weight})")
            if failure_modes:
                context_parts.append(f"COMMON FAILURE MODES: {', '.join(failure_modes)}")

        # Error patterns
        if error_patterns:
            context_parts.append("SPECIFIC ERRORS MADE:")
            for pattern in error_patterns[:5]:  # Limit to 5
                context_parts.append(f"  - {pattern}")

        # Cognitive state
        if cognitive_state:
            encoding = cognitive_state.get("encoding", 0.5)
            fatigue = cognitive_state.get("fatigue", 0)
            if encoding < 0.5:
                context_parts.append(f"NOTE: Low encoding ({encoding:.0%}) - learner may need foundational explanations")
            if fatigue > 0.7:
                context_parts.append(f"NOTE: High fatigue ({fatigue:.0%}) - keep notes concise and scannable")

        context_str = "\n".join(context_parts) if context_parts else "No specific context provided"

        # Adjust depth instruction
        depth_instructions = {
            "brief": "Keep notes VERY concise - just the specific point(s) missed. 1-2 paragraphs max.",
            "standard": "Provide KEY concepts, misconceptions, and a quick reference. Medium length.",
            "comprehensive": "Provide FULL coverage with analogies, mnemonics, detailed tables, and command examples.",
        }
        ...
        prompt = f"""Generate study notes for this CCNA section.
...
Now generate Markdown study notes following the format from your instructions.
Focus on what the learner got WRONG and needs to remember."""

        try:
            # Use a different model configuration for notes (more creative)
            ...
            if notes.startswith("```markdown"):
                notes = notes[len("```markdown"):].strip()
            if notes.startswith("```"):
                notes = notes[3:].strip()
            if notes.endswith("```"):
                notes = notes[:-3].strip()

            logger.info(f"Generated {len(notes)} chars of study notes for {section_id}")
            return notes

        except Exception as e:
            logger.error(f"Study notes generation failed for '{section_id}': {e}")
