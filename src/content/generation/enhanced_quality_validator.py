...
MALFORMED_PATTERNS = [
    ...
    (
        re.compile(r"what\s+\w+\s+The\s+\w+", re.IGNORECASE),
        "GARBLED_WHAT_THE",
        "Question contains 'what X The Y' garbled pattern",
    ),
    (
        re.compile(r"what\s+\w+\s+However\b", re.IGNORECASE),
        "GARBLED_HOWEVER",
        "Question contains 'what X However' fragment",
    ),
    (
        re.compile(r"what\s+\w+\s+If\s+", re.IGNORECASE),
        "GARBLED_IF",
        "Question contains 'what X If' fragment",
    ),
    (
        re.compile(r"what\s+\w+\s+Some\s+", re.IGNORECASE),
        "GARBLED_SOME",
        "Question contains 'what X Some' fragment",
    ),
    (
        re.compile(r"what\s+is\s+This\b(?!\s+called)", re.IGNORECASE),
        "VAGUE_THIS",
        "Question uses vague 'What is This'",
    ),
    (
        re.compile(r"what\s+type\s+This\s+type", re.IGNORECASE),
        "GARBLED_TYPE",
        "Question contains 'what type This type' pattern",
    ),
    # Questions about formatting elements
    (
        re.compile(r"what\s+is\s+Note:\s*\?", re.IGNORECASE),
        "FORMAT_NOTE",
        "Question about markdown Note: element",
    ),
    (
        re.compile(r"what\s+is\s+Step\s+\d", re.IGNORECASE),
        "FORMAT_STEP",
        "Question about step number",
    ),
    (
        re.compile(r"what\s+is\s+Example", re.IGNORECASE),
        "FORMAT_EXAMPLE",
        "Question about Example: element",
    ),
    # Repeated words (sign of generation error)
    (
        re.compile(r"\b(\w{4,})\s+\1\b", re.IGNORECASE),
        "REPEATED_WORD",
        "Contains repeated word (generation error)",
    ),
]
...
TRUNCATION_PATTERNS = [
    ...
    (
        re.compile(r"\s+(a|an|the)\s*$", re.IGNORECASE),
        "TRUNCATED_ARTICLE",
        "Ends with article (truncated)",
    ),
    (
        re.compile(r"\s+(of|to|for|with|by|at|in|on|from)\s*$", re.IGNORECASE),
        "TRUNCATED_PREPOSITION",
        "Ends with preposition (truncated)",
    ),
    (
        re.compile(r"\s+(or|and|but|nor)\s*$", re.IGNORECASE),
        "TRUNCATED_CONJUNCTION",
        "Ends with conjunction (truncated)",
    ),
    (
        re.compile(r"\s+(is|are|was|were|be|been|being)\s*$", re.IGNORECASE),
        "TRUNCATED_VERB",
        "Ends with linking verb (truncated)",
    ),
...
QUALITY_WARNING_PATTERNS = [
    ...
    (
        re.compile(r"^Define\s+\w+\.?\s*$", re.IGNORECASE),
        "BARE_DEFINE",
        "Simple 'Define X' lacks context",
    ),
    # Very short content
    ...
    (
        ...
        "WEAK_DISTRACTOR_ALL",
        "'All of the above' is weak",
    ),
    (
        re.compile(r"none\s+of\s+the\s+above", re.IGNORECASE),
        "WEAK_DISTRACTOR_NONE",
        "'None of the above' is weak",
    ),
    (
        re.compile(r"both\s+[A-D]\s+and\s+[A-D]", re.IGNORECASE),
        "WEAK_DISTRACTOR_BOTH",
        "'Both X and Y' is weak",
    ),
]
...
class EnhancedQualityValidator:
    ...
    def __init__(
        ...
        if use_perplexity:
            try:
                from .quality_validator import PerplexityScorer
        ...
        if use_grammar:
            try:
                from .quality_validator import GrammarValidator
    ...
    def validate(
        ...
        # Pass 7b: Notation/math checks (IP/CIDR/VLAN ranges)
        self._check_notation(front, back, result)
    ...
    def _check_type_specific(
        ...
        if atom_type == AtomType.MCQ:
            ...
            self._validate_numeric(content_json, result)
        elif atom_type == AtomType.ORDERING:
            self._validate_ordering(content_json, result)
        elif atom_type == AtomType.LABELING:
            self._validate_labeling(content_json, result)
        elif atom_type == AtomType.HOTSPOT:
            self._validate_hotspot(content_json, result)
        elif atom_type == AtomType.CASE_SCENARIO:
            self._validate_case_scenario(content_json, result)
    ...
    def _check_notation(
        ...
        """Validate IP/CIDR and basic network notation."""
        combined = f"{front} {back}"
        ip_patterns = re.findall(r"\b\d{1,3}(?:\.\d{1,3}){3}(?:/\d{1,2})?\b", combined)
        for candidate in ip_patterns:
            try:
                # ip_interface handles both plain IPs and CIDR notation
                ipaddress.ip_interface(candidate)
            except ValueError:
                result.add_issue(
                    code="NOTATION_IP_INVALID",
                    severity=ValidationSeverity.ERROR,
                    message=f"Invalid IP/CIDR notation: {candidate}",
                    field="back",
                    penalty=30,
                )

    def _validate_mcq(self, content_json: dict | None, result: EnhancedValidationResult):
        ...
        for i, option in enumerate(options):
            ...
            option_lower = str(option_text).lower()
    ...
    def _validate_numeric(self, content_json: dict | None, result: EnhancedValidationResult):
        """Validate Numeric atom structure."""
        if not content_json:
            result.add_issue(
                code="NUMERIC_NO_CONTENT",
                severity=ValidationSeverity.ERROR,
                message="Numeric requires content_json",
                penalty=50,
            )
            return
        if "correct_value" not in content_json:
            result.add_issue(
                code="NUMERIC_NO_VALUE",
                severity=ValidationSeverity.ERROR,
                message="Numeric must have correct_value",
                penalty=50,
            )
        tol = content_json.get("tolerance")
        if tol is not None:
            try:
                float(tol)
            except Exception:
                result.add_issue(
                    code="NUMERIC_BAD_TOLERANCE",
                    severity=ValidationSeverity.WARNING,
                    message="tolerance must be numeric",
                    penalty=5,
                )
        # Units optional; no-op

    def _validate_ordering(self, content_json: dict | None, result: EnhancedValidationResult):
        """Validate Ordering atom structure."""
        if not content_json:
            result.add_issue(
                code="ORDERING_NO_CONTENT",
                severity=ValidationSeverity.ERROR,
                message="Ordering requires content_json",
                penalty=50,
            )
            return
        options = content_json.get("options", [])
        ...
        if not options:
            result.add_issue(
                code="ORDERING_NO_OPTIONS",
                severity=ValidationSeverity.ERROR,
                message="Ordering must have options",
                penalty=50,
            )
        if not correct_order:
            result.add_issue(
                code="ORDERING_NO_ORDER",
                severity=ValidationSeverity.ERROR,
                message="Ordering must specify correct_order",
                penalty=50,
            )
        # Validate that order references are valid (indices or ids)
        option_ids = {o.get("id") for o in options if isinstance(o, dict) and o.get("id") is not None}
        for ref in correct_order:
            if isinstance(ref, int):
                if not (0 <= ref < len(options)):
                    result.add_issue(
                        code="ORDERING_BAD_INDEX",
                        severity=ValidationSeverity.ERROR,
                        message=f"Index {ref} out of range",
                        penalty=40,
                    )
            else:
                if ref not in option_ids:
                    result.add_issue(
                        code="ORDERING_UNKNOWN_ID",
                        severity=ValidationSeverity.ERROR,
                        message=f"Unknown option id '{ref}' in correct_order",
                        penalty=40,
                    )

    def _validate_labeling(self, content_json: dict | None, result: EnhancedValidationResult):
        """Validate Labeling atom structure."""
        if not content_json:
            result.add_issue(
                code="LABELING_NO_CONTENT",
                severity=ValidationSeverity.ERROR,
                message="Labeling requires content_json",
                penalty=50,
            )
            return
        targets = content_json.get("targets", [])
        labels = content_json.get("labels", [])
        if not targets:
            result.add_issue(
                code="LABELING_NO_TARGETS",
                severity=ValidationSeverity.ERROR,
                message="Labeling must define targets",
                penalty=50,
            )
        # Ensure targets have id and label
        for t in targets:
            if not isinstance(t, dict) or ("id" not in t or "label" not in t):
                result.add_issue(
                    code="LABELING_BAD_TARGET",
                    severity=ValidationSeverity.ERROR,
                    message="Each target must include id and label",
                    penalty=40,
                )
        # If label bank provided, ensure coverage
        if labels:
            target_labels = {t.get("label") for t in targets if isinstance(t, dict)}
            bank = set(labels) if isinstance(labels, list) else set()
            missing = {l for l in target_labels if l and l not in bank}
            if missing:
                result.add_issue(
                    code="LABELING_LABEL_BANK_MISSING",
                    severity=ValidationSeverity.WARNING,
                    message=f"Label bank missing labels: {sorted(missing)}",
                    penalty=10,
                )

    def _validate_hotspot(self, content_json: dict | None, result: EnhancedValidationResult):
        """Validate Hotspot atom structure."""
        if not content_json:
            result.add_issue(
                code="HOTSPOT_NO_CONTENT",
                severity=ValidationSeverity.ERROR,
                message="Hotspot requires content_json",
                penalty=50,
            )
            return
        hotspots = content_json.get("hotspots", [])
        if not hotspots:
            result.add_issue(
                code="HOTSPOT_NO_REGIONS",
                severity=ValidationSeverity.ERROR,
                message="Hotspot must define hotspots (regions/ids)",
                penalty=50,
            )
        choices = content_json.get("choices")
        if choices:
            # If discrete choices provided, ensure at least include all hotspots
            missing = [h for h in hotspots if h not in choices]
            if missing:
                result.add_issue(
                    code="HOTSPOT_CHOICES_INCOMPLETE",
                    severity=ValidationSeverity.WARNING,
                    message="choices should include all hotspot answers",
                    penalty=10,
                )

    def _validate_case_scenario(self, content_json: dict | None, result: EnhancedValidationResult):
        """Validate Case Scenario atom structure."""
        if not content_json:
            result.add_issue(
                code="CASE_NO_CONTENT",
                severity=ValidationSeverity.ERROR,
                message="case_scenario requires content_json",
                penalty=50,
            )
            return
        ...
        if not steps:
            result.add_issue(
                code="CASE_NO_STEPS",
                severity=ValidationSeverity.ERROR,
                message="case_scenario must include steps",
                penalty=50,
            )
        else:
            for i, step in enumerate(steps):
                ...
                if not step.get("prompt"):
                    result.add_issue(
                        code="CASE_STEP_NO_PROMPT",
                        severity=ValidationSeverity.ERROR,
                        message=f"Step {i} missing prompt",
                        penalty=30,
                    )
                options = step.get("options", [])
                if not options:
                    result.add_issue(
                        code="CASE_STEP_NO_OPTIONS",
                        severity=ValidationSeverity.ERROR,
                        message=f"Step {i} missing options",
                        penalty=30,
                    )
                ci = step.get("correct_index")
                if ci is None or not isinstance(ci, int) or not (0 <= ci < len(options)):
                    result.add_issue(
                        code="CASE_STEP_BAD_CORRECT_INDEX",
                        severity=ValidationSeverity.ERROR,
                        message=f"Step {i} has invalid correct_index",
                        penalty=30,
                    )
        if not content_json.get("summary"):
            result.add_issue(
                code="CASE_NO_SUMMARY",
                severity=ValidationSeverity.WARNING,
                message="case_scenario should include a summary",
                penalty=5,
            )

    def _validate_true_false(self, content_json: dict | None, result: EnhancedValidationResult):
    ...
    def _validate_matching(self, content_json: dict | None, result: EnhancedValidationResult):
    ...
    def _validate_parsons(self, content_json: dict | None, result: EnhancedValidationResult):
    ...
    def _validate_flashcard(self, front: str, back: str, result: EnhancedValidationResult):
        ...
        if front and not front.strip().endswith("?"):
            ...
            if any(
                front.lower().startswith(w)
                for w in ["what", "how", "why", "when", "where", "which", "who"]
            ):
                result.add_issue(
    ...
    def _check_quality_warnings(
        ...
        for pattern, code, message in QUALITY_WARNING_PATTERNS:
            if pattern.search(combined):
                severity = (
                    ValidationSeverity.ERROR if code in escalate_to_error else ValidationSeverity.WARNING
                )
                result.add_issue(
                    code=code,
                    severity=severity,
                    message=message,
                    penalty=20 if severity == ValidationSeverity.ERROR else 10,
                )
        ...
        for marker in multi_fact_markers:
            if re.search(marker, back, re.IGNORECASE):
                result.add_issue(
                    ...
                    severity=ValidationSeverity.WARNING,
                    message="Answer contains multi-fact marker",
                    penalty=10,
                )
                result.is_atomic = False
                break
...
class NumberSystemValidator:
    ...
    BINARY_PATTERN = re.compile(
        r"\b([01]{4,8}(?:\.[01]{8}){0,3})\b"
    )  # e.g., 11000000 or 11000000.10101000.00001010.00001010
    ...
    CONVERSION_PATTERNS = [
        ...
        # "binary X = decimal Y"
        re.compile(
            r"[`\"']?([01]{8})[`\"']?\s*(?:=|→|is)\s*(?:\*\*)?(\d{1,3})(?:\*\*)?", re.IGNORECASE
        ),
        # "Convert X to binary... Result: Y"
        re.compile(
            r"convert\s+(\d{1,3}).*?(?:result|=|→)[:\s]*[`\"']?([01]{8})[`\"']?",
            re.IGNORECASE | re.DOTALL,
        ),
        # Hex conversions: "X in hex: Y" or "hex: Y"
        re.compile(
            r"(\d{1,3})\s*(?:in\s+hex(?:adecimal)?|→|=)\s*[`\"']?([0-9A-Fa-f]{2})[`\"']?",
            re.IGNORECASE,
        ),
        # "hex X = decimal Y"
        re.compile(
            r"[`\"']?([0-9A-Fa-f]{2})[`\"']?\s*(?:=|→|is)\s*(?:\*\*)?(\d{1,3})(?:\*\*)?",
            re.IGNORECASE,
        ),
    ]
        ...
        try:
            ...
            for i, (bin_oct, dec_oct) in enumerate(zip(binary_octets, decimal_octets)):
                ...
                if expected_decimal != actual_decimal:
                    return (
                        False,
                        f"Octet {i + 1}: {bin_oct} = {expected_decimal}, not {actual_decimal}",
                    )
        ...
        for pattern in self.CONVERSION_PATTERNS:
            ...
            for match in matches:
                ...
                if len(groups) >= 2:
                    ...
                    if all(c in "01" for c in val1) and not val1.isdigit():
                        try:
                            expected = int(val1, 2)
                            actual = int(val2)
                            if expected != actual:
...
_validator_instance: EnhancedQualityValidator | None = None
_math_validator_instance: NumberSystemValidator | None = None
...
if __name__ == "__main__":
    ...
    test_cases = [
        # Good examples
        (
            "What routing protocol uses Dijkstra's algorithm?",
            "OSPF uses Dijkstra's shortest path first algorithm to calculate the best route to each destination.",
            "flashcard",
        ),
        (
            "What is the purpose of a VLAN in enterprise networks?",
            "VLANs logically segment a physical network into separate broadcast domains, improving security and reducing congestion.",
            "flashcard",
        ),
    ...
    for front, back, atom_type in test_cases:
        content = (
            {"options": ["TCP", "IP", "Ethernet", "ARP"], "correct_index": 0}
            if atom_type == "mcq"
            else None
        )
    ...
    math_test_cases = [
        ...
        ("Convert D2 to decimal.", "D2 in hex is 210 in decimal.", True, "Correct hex to decimal"),
        (
