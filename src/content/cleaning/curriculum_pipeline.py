...
class CurriculumPipeline:
    ...
    def process_module(
        ...
        try:
            ...
            misplaced = [
                c for c in classified if c.suggested_module != module_name and c.suggested_module
            ]
    ...
    def _get_module_info(self, module_name: str) -> tuple[UUID | None, list | None]:
        """
        ...
        logger.warning(
            f"Module '{module_name}' not found in database, using name for semantic matching"
        )
    ...
    def _fetch_anki_cards(self, deck_name: str | None = None) -> list[dict[str, Any]]:
    ...
    def _classify_cards(
        ...
        for card in cards:
            ...
            # Use semantic similarity
                ...
                similarity = float(
                    np.dot(card_emb, mod_emb) / (np.linalg.norm(card_emb) * np.linalg.norm(mod_emb))
                )
    ...
    def _find_best_module(self, card_embedding) -> dict | None:
    ...
    def _analyze_quality(self, cards: list[CardAssignment]) -> dict[str, Any]:
        ...
        from .atomicity import CardQualityAnalyzer
