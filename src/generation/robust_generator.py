...
class RobustAtomGenerator:
    ...
    def extract_content(
        ...
        content.has_procedures = bool(content.cli_commands) or \
            bool(re.search(r"step\s+\d|first.*then|configure.*by", raw_text, re.IGNORECASE))
        ...
        for atom_type in target_types:
            try:
                ...
                for atom in atoms:
                    ...
                    elif validation.can_be_fixed and atom.generation_attempt < self.MAX_REGENERATION_ATTEMPTS:
                        # Stage 5: Retry
                ...
                # Track by type
                result.by_type[atom_type] = len([a for a in result.atoms if a.atom_type == atom_type])
    ...
    def _item_to_atom(
        ...
        try:
            ...
            card_id = item.get(
                "card_id",
                f"{section_id}-{atom_type[:2].upper()}-{index+1:03d}"
            )
