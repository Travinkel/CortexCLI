...
with engine.connect() as conn:
    ...
    for row in result:
        print(f"  {row.section_id}: {row.title[:40]:40} content={row.content_len:6} terms={row.terms_len}")
    ...
    for row in result:
        print(f"  {row.section_id}: FC={row.flashcards} TF={row.tf} CL={row.cloze} MCQ={row.mcq} Total={row.total}")
