...
class MCQGenerator:
    ...
    def __init__(self, api_key: str | None = None):
        ...
        if HAS_GENAI and self.api_key:
            ...
            self.model = genai.GenerativeModel(
                model_name=self.model_name, system_instruction=MCQ_SYSTEM_PROMPT
            )
            ...
            if not HAS_GENAI:
                ...
                logger.error(
                    "No Gemini API key found. Set GEMINI_API_KEY or gemini_api_key in .env"
                )

    def generate(self, concept_name: str, atom_text: str) -> GeneratedMCQ | None:
    ...
    def _generate_with_llm(self, concept_name: str, atom_text: str) -> GeneratedMCQ | None:
        """Generate MCQ using Gemini API."""
        try:
            prompt = MCQ_USER_TEMPLATE.format(concept_name=concept_name, atom_text=atom_text)
...
def insert_mcq(conn, mcq: GeneratedMCQ, source_atom: dict) -> str | None:
...
def main():
    ...
    parser.add_argument(
        "--batch-delay", type=float, default=0.5, help="Delay between API calls (rate limiting)"
    )
    ...
    with engine.connect() as conn:
        ...
        for atom in atoms:
            ...
            # Generate MCQ
            mcq = generator.generate(concept_name=atom["concept_name"], atom_text=atom["back"])
