...
def batch_regenerate_tf(conn, generator: BatchGenerator, limit: int, dry_run: bool) -> dict:
    ...
    for row in rows:
        ...
        if new_data and "front" in new_data and "answer" in new_data:
            logger.info(
                f"[{stats['processed']}/{len(rows)}] Regenerated: {new_data['front'][:60]}..."
            )
        ...
        if not dry_run and stats["processed"] % 50 == 0:
            conn.commit()
            logger.info("Committed batch of 50...")
...
def main():
    ...
    parser.add_argument(
        "--quiz", type=int, default=0, help="Generate MCQ/matching/parsons for N sections"
    )
    ...
    with engine.connect() as conn:
        if args.tf > 0:
            ...
            logger.info(
                f"T/F: {stats['regenerated']}/{stats['processed']} regenerated, {stats['failed']} failed"
            )
        ...
        if args.flashcards > 0:
            ...
            logger.info(
                f"Flashcards: {stats['regenerated']}/{stats['processed']} regenerated, {stats['failed']} failed"
            )
        ...
        if args.quiz > 0:
            ...
            logger.info(
                f"Quiz: {stats['mcqs_created']} MCQs, {stats['matching_created']} matching, {stats['parsons_created']} parsons"
            )
