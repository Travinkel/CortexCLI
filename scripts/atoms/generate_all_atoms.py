...
def extract_section_content(module_path: Path, section_id: str) -> SectionContent | None:
    ...
    for sentence in sentences:
        ...
        if re.search(
            r"\b(?:is|are|refers to|means|called|known as|defined as)\b", sentence, re.IGNORECASE
        ):
    ...
    comparison_pattern = (
        r"([A-Z][a-z]+(?:\s+[a-z]+)*)\s+(?:vs\.?|versus|and|or)\s+([A-Z][a-z]+(?:\s+[a-z]+)*)"
    )
...
def generate_flashcards(section: SectionContent) -> list[LearningAtom]:
    ...
    for fact in section.facts:
        ...
        if numbers:
            ...
            fact_question = re.sub(
                r"\d+(?:\.\d+)?\s*(?:Mbps|Gbps|MHz|GHz|Kbps)",
                "___",
                fact,
                count=1,
                flags=re.IGNORECASE,
        ...
        if is_match:
            ...
            if (
                ...
                and not re.search(r"[,]{2,}", subject)  # No repeated commas
                and not subject.lower().startswith("the ")  # Avoid "The concept of..."
                and len(subject.split()) < 10
            ):  # Not too many words
                atoms.append(
                    LearningAtom(
                        id=str(uuid.uuid4()),
    ...
    for fact in section.facts:
        ...
        if bold_in_fact:
            ...
            networking_terms = re.findall(
                r"\b((?:IP|TCP|UDP|LAN|WAN|MAC|DNS|DHCP|HTTP|FTP|SMTP|ICMP|ARP|NAT|VLAN|VPN|OSI|PDU|MTU|TTL)[A-Za-z]*)\b",
                fact,
                re.IGNORECASE,
            )
            if networking_terms:
                ...
                cloze_fact = re.sub(
                    rf"\b{re.escape(term)}\b", "{{c1::" + term + "}}", fact, count=1
                )
                atoms.append(
                    LearningAtom(
                        id=str(uuid.uuid4()),
    ...
    for item_list in section.lists:
        for item in item_list[:5]:
            ...
            if key_terms and len(key_terms[0]) > 2:
                term = key_terms[0]
                cloze_item = re.sub(
                    rf"\b{re.escape(term)}\b", "{{c1::" + term + "}}", item, count=1
                )
                if "{{c1::" in cloze_item:
                    atoms.append(
                        LearningAtom(
            ...
            atoms.append(
                LearningAtom(
                    ...
                    back=json.dumps(
                        {
                            "answer": True,
                            "explanation": "This is a correct statement from the CCNA material.",
                        }
                    ),
                    section_id=section.section_id,
                    module_number=section.module_number,
    ...
    for acronym, expansion in section.acronyms:
        atoms.append(
            LearningAtom(
                ...
                back=json.dumps(
                    {"answer": True, "explanation": f"Correct. {acronym} = {expansion}"}
                ),
                section_id=section.section_id,
                module_number=section.module_number,
    ...
    if len(section.commands) >= 3:
        ...
        atoms.append(
            LearningAtom(
                id=str(uuid.uuid4()),
                atom_type="parsons",
                front="Arrange these CLI commands in the correct order:",
                back=json.dumps(
                    {
...
def main():
    ...
    with engine.connect() as conn:
        if args.section:
            ...
            result = conn.execute(
                text(
                    "SELECT section_id, module_number FROM ccna_sections WHERE module_number = :mod AND level = 3 ORDER BY display_order"
                ),
                {"mod": args.module},
            )
        elif args.all:
            ...
            result = conn.execute(
                text(
                    "SELECT section_id, module_number FROM ccna_sections ORDER BY module_number, display_order"
                )
            )
        else:
