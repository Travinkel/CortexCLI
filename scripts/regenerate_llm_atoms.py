...
def clear_llm_atoms(source: str = "llm-generated") -> int:
    ...
    with engine.connect() as conn:
        result = conn.execute(text("""
            DELETE FROM clean_atoms WHERE source = :source
        """), {"source": source})
        conn.commit()
...
def main():
    ...
    parser.add_argument(
        "--module", type=int, help="Generate for specific module (1-17)"
    )
    parser.add_argument(
        "--all", action="store_true", help="Generate for all modules"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Preview without saving"
    )
    parser.add_argument(
        "--max-per-section", type=int, default=8,
        help="Max cards per section (default: 8)"
    )
    parser.add_argument(
    ...
    generator = LLMFlashcardGenerator(
        use_quality_validation=not args.no_validation
    )
    ...
    with Progress(
        ...
        for section in all_sections:
            ...
            progress.update(
                task,
                description=f"Module {module_num}: {section_id} {title[:30]}..."
            )
