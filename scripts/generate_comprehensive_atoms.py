...
def get_section_coverage() -> dict[str, SectionCoverage]:
    ...
    with engine.connect() as conn:
        # Get sections
        sections_result = conn.execute(text("""
...
def main():
    ...
    parser.add_argument("--under-covered", action="store_true", help="Only generate for under-covered sections")
    parser.add_argument("--dry-run", action="store_true", help="Preview without generating")
    parser.add_argument("--report", action="store_true", help="Show coverage report only")
    parser.add_argument("--target", type=int, default=10, help="Target atoms per section (default: 10)")
    ...
    if args.module:
        sections_to_process = [
            c for c in coverage.values()
            if c.module_number == args.module and c.needs_more
        ]
