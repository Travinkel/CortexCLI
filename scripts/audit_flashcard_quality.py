...
def audit_flashcards(conn, fix: bool = False) -> dict:
    ...
    result = conn.execute(text(
        "SELECT COUNT(*) FROM clean_atoms WHERE atom_type = 'flashcard'"
    ))
    total = result.scalar()
        ...
        try:
            ...
            if count > 0:
                issues_found[issue_id] = {
                    ...
                    "action": issue["action"],
                    "samples": [(str(r.id), r.front[:50], r.back[:30] if r.back else "") for r in rows[:5]]
                }
    ...
    for issue_id, data in sorted(issues_found.items(), key=lambda x: {"critical": 0, "high": 1, "medium": 2, "low": 3}[x[1]["severity"]]):
