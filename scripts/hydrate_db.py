...
from src.db.database import engine, init_db, session_scope
...
def map_atom_to_clean_atom(
    ...
    fidelity_meta = extract_fidelity_metadata(atom_data)
...
def fetch_atoms_from_db(
    ...
    with session_scope() as session:
        ...
        source_exists = session.execute(text("""
            SELECT EXISTS (
        ...
        for atom_data in atoms:
            ...
            try:
                ...
                if stats.atoms_processed % 500 == 0:
                    session.commit()
                    logger.info(f"Progress: {stats.atoms_processed} atoms processed, {stats.quiz_questions_created} created")
...
def hydrate_clean_atoms(
    ...
    logger.info(f"Found {len(module_map)} modules, {len(section_concept_map)} section-concept links")
        ...
        for atom_data in atoms:
            ...
            try:
                ...
                if existing:
                    ...
                    update_cols = ", ".join(
                        f"{k} = :{k}" for k in clean_atom_values.keys()
                        if k != "card_id"
                    )
                    session.execute(
                        text(f"""
                ...
                if quiz_values:
                    ...
                    if existing_quiz:
                        ...
                        update_cols = ", ".join(
                            f"{k} = :{k}" for k in quiz_values.keys()
                            if k != "atom_id"
                        )
                        session.execute(
                            text(f"""
...
def print_table_counts() -> None:
    ...
    with session_scope() as session:
        clean_atoms_count = session.execute(
            text("SELECT COUNT(*) FROM clean_atoms")
        ).scalar()

        quiz_questions_count = session.execute(
...
Examples:
  ...
  python scripts/hydrate_db.py --from-json --json-dir output/
    ...
    parser.add_argument(
        "--from-json",
        action="store_true",
        help="Read from JSON files instead of database"
    )
    parser.add_argument(
        "--json-dir",
        type=Path,
    ...
    parser.add_argument(
        "--module",
        type=str,
        help="Process only specific module (e.g., NET-M1)"
    )
    parser.add_argument(
        "--grade-filter",
        type=str,
    ...
    parser.add_argument(
        "--init-db",
        action="store_true",
        help="Initialize database tables before hydrating"
    )
    parser.add_argument(
        "--quiz-only",
        action="store_true",
