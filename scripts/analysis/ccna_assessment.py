...
class CCNAModuleParser:
    ...
    def parse_all_modules(self) -> dict[int, ModuleAnalysis]:
        ...
        for txt_file in sorted(self.ccna_dir.glob("CCNA Module *.txt")):
            ...
            if match:
                ...
                logger.info(
                    f"Parsed Module {module_num}: {analysis.title} ({analysis.line_count} lines)"
                )
    ...
    def parse_module(self, file_path: Path, module_num: int) -> ModuleAnalysis:
        ...
        for subsection_num, subsection_title in subsections:
            if not any(
                skip in subsection_title.lower()
                for skip in ["example", "check", "packet tracer", "syntax"]
            ):
...
class DatabaseAssessor:
    ...
    def detect_hallucinations(
        self, cards: list[dict], module_content: dict[int, ModuleAnalysis]
    ) -> list[dict]:
        """
...
def main():
    ...
    parser.add_argument(
        "--ccna-dir", type=Path, default=Path("docs/CCNA"), help="CCNA modules directory"
    )
