"""Merge 200 massive matching atoms into cortex_master.json."""
import json
import uuid
from pathlib import Path

def main():
    atoms_file = Path("outputs/generated_atoms/massive_matching.json")
    if not atoms_file.exists():
        print("ERROR: massive_matching.json not found")
        return

    with open(atoms_file, "r", encoding="utf-8") as f:
        new_atoms = json.load(f)

    print(f"Loaded {len(new_atoms)} matching atoms")
    ...
    for atom in new_atoms:
        card_id = atom.get("card_id", f"MAT-{uuid.uuid4().hex[:8].upper()}")
    ...
    print(f"\nAdded {added} new matching atoms")
    print(f"Total deck size: {len(existing_atoms)} atoms")

    matching_count = sum(1 for a in existing_atoms if a.get("atom_type") == "matching")
    print(f"\nTOTAL MATCHING ATOMS: {matching_count}")
