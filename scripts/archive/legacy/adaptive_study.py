...
def get_track_id(session) -> UUID | None:
            ...
            if concepts:
                ...
                for c in concepts:
                    status = (
                        ...
                        if c.is_unlocked
                        else Colors.RED + "✗" + Colors.ENDC
                    )
                    mastery = float(c.combined_mastery or 0)
                    print(
                        f"{c.name[:48]:<50} | {print_mastery_bar(mastery, 10):>12} | {c.review_count or 0:>8} {status}"
                    )
...
def cmd_gaps(args):
    ...
    for i, gap in enumerate(gaps[:10], 1):
        ...
        print(
            f"   Current: {gap.current_mastery * 100:.1f}% → Target: {gap.required_mastery * 100:.1f}%"
        )
...
def cmd_study(args):
    ...
    if not session_state.current_atom:
        print(
            f"{Colors.YELLOW}No atoms available for study. Generate content first.{Colors.ENDC}\n"
        )
        return
            ...
            if is_correct:
                ...
                if result.remediation_triggered:
                    ...
                    if result.remediation_plan:
                        ...
                        print(
                            f"Recommended: Review {len(result.remediation_plan.atoms)} related cards"
                        )

                        inject = (
                            input("Would you like to review prerequisite content? (y/n): ")
                            .strip()
                            .lower()
                        )
                        if inject in ["y", "yes"]:
...
def main():
    parser = argparse.ArgumentParser(description="Adaptive Study CLI - Full-loop adaptive learning")
