...
def get_clusters(session) -> list[dict]:
    ...
    return [
        {"id": str(row.id), "name": row.name, "count": row.atom_count} for row in result.fetchall()
    ]


def get_study_atoms(session, cluster_id: str | None = None, limit: int = 20) -> list[dict]:
...
def run_study_session(learner_id: str, cluster_id: str | None = None, max_cards: int = 20):
    ...
    with session_scope() as session:
        ...
        print(
            f"{Colors.DIM}Controls: [Enter]=Show answer, [y]=Correct, [n]=Incorrect, [q]=Quit{Colors.END}\n"
        )
            ...
            for i, atom in enumerate(atoms, 1):
                ...
                while True:
                    ...
                    if rating in ["y", "n", "q", ""]:
                        break
                    print(
                        f"{Colors.RED}Please enter y (correct), n (incorrect), or q (quit){Colors.END}"
                    )
...
def show_cluster_menu():
    ...
    with session_scope() as session:
        ...
        while True:
            try:
                choice = input(
                    f"{Colors.BOLD}Enter number (0-{len(clusters)}): {Colors.END}"
                ).strip()
                if not choice:
                    choice = "0"
                idx = int(choice)
