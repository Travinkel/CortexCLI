...
# Add project root to path (scripts/anki -> scripts -> repo root)
...
def create_decks_batch(deck_names: list) -> int:
    ...
    actions = [{"action": "createDeck", "params": {"deck": name}} for name in deck_names]
...
def get_all_anki_notes() -> dict:
    ...
    for note in notes_info:
        ...
        card_id = fields.get("Card ID", {}).get("value", "") or fields.get("CardID", {}).get(
            "value", ""
        )
        if card_id:
...
def pull_anki_stats():
    ...
    with session_scope() as session:
        for card in cards_info:
            ...
            fields = note.get("fields", {})
            card_id_field = fields.get("Card ID", {}).get("value", "") or fields.get(
                "CardID", {}
            ).get("value", "")
    ...
    with session_scope() as session:
        ...
        query = f"""
            SELECT
                ...
                -- Prefer explicit module mapping, fall back to section mapping
                COALESCE(lm.week_order, cs.module_number) as module_number,
                cs.module_number as section_module_number,
        ...
        logger.info(
            f"Quality filter: score >= {ANKI_MIN_QUALITY_SCORE}, front >= {ANKI_MIN_FRONT_LENGTH} chars, back >= {ANKI_MIN_BACK_LENGTH} chars"
        )
        ...
        if not dry_run:
            ...
            for i in range(0, len(sorted_decks), batch_size):
                ...
                logger.info(
                    f"  Created {min(i + batch_size, len(sorted_decks))}/{len(deck_names_needed)} decks..."
                )
        ...
        for i, atom in enumerate(atoms):
            ...
            # Prefer section-derived module when present to avoid mismatches
            module_num = (
                atom.get("section_module_number")
                or atom.get("module_number")
                or 0
            )
            deck_name = get_module_deck_name(module_num)
            ...
            # Section tag (struggle-aware filtering)
            section_id = atom.get("section_id") or atom.get("ccna_section_id")
            if section_id:
                tags.append(f"{CURRICULUM_ID}:{section_id}")
            ...
            metadata = {
                ...
                "section_title": atom.get("section_title"),
        ...
        if notes_to_add:
            ...
            if notes_to_add:
                ...
                logger.debug("Sample note structure:")
                logger.debug(f"  deckName: {sample.get('deckName')}")
                logger.debug(f"  modelName: {sample.get('modelName')}")
                logger.debug(
                    f"  fields.front: {sample.get('fields', {}).get('front', 'MISSING')[:80]}..."
                )
                logger.debug(
                    f"  fields.back: {sample.get('fields', {}).get('back', 'MISSING')[:50]}..."
                )
            ...
            for i in range(0, len(notes_to_add), batch_size):
                ...
                try:
                    ...
                    if batch:
                        logger.debug(
                            f"First note in batch: {json.dumps(batch[0], indent=2, default=str)[:500]}"
                        )
                    # Try one by one
                    for note in batch:
                        try:
