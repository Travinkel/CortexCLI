...
def anki_invoke(action: str, params: dict = None, timeout: int = 30) -> dict:
    ...
    request_json = json.dumps({"action": action, "version": 6, "params": params or {}}).encode(
        "utf-8"
    )
...
def main():
    ...
    parser.add_argument(
        "--semantic-match", action="store_true", help="Use semantic matching for unmatched cards"
    )
    parser.add_argument(
        "--transfer-mastery", action="store_true", help="Update concept mastery scores"
    )
        ...
        if not args.dry_run:
            ...
            if args.semantic_match and stats["unmatched"] > 0:
                ...
                for m in matches[:10]:  # Show first 10
                    logger.info(
                        f"  Match ({m['similarity']:.2f}): '{m['anki_front']}...' → '{m['matched_front']}...'"
                    )
