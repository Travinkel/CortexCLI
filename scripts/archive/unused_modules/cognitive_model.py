...
class CognitiveDiagnosis:
    ...
    related_concept_id: UUID | None = None  # For interference
    source_reference: str | None = None  # Section to re-read
...
def diagnose_error(
    ...
    if response_time_ms < THRESHOLDS["impulsivity_max_ms"]:
        ...
        evidence_item = (
            f"Response time {response_time_ms}ms < {THRESHOLDS['impulsivity_max_ms']}ms threshold"
        )
        candidates.append(
            (
    ...
    if fatigue_signals >= 2:
        candidates.append(
            (CognitiveState.FATIGUE, min(0.9, fatigue_signals * 0.3), fatigue_evidence)
        )
    ...
    if review_count < THRESHOLDS["encoding_min_reviews"] and stability < 7:
        ...
        candidates.append(
            (CognitiveState.ENCODING_FAILURE, max(0.3, encoding_confidence), encoding_evidence)
        )
    ...
    if concept_id and history:
        ...
        if interference_evidence:
            candidates.append((CognitiveState.INTERFERENCE, 0.7, interference_evidence))
...
def _detect_interference(atom: dict, history: list[dict]) -> list[str]:
    ...
    if atom_module:
        ...
        if same_module_errors:
            evidence.append(f"Multiple errors in Module {atom_module} on different concepts")
...
def _set_remediation(diagnosis: CognitiveDiagnosis, atom: dict) -> None:
    ...
    if diagnosis.state == CognitiveState.IMPULSIVITY:
        ...
        diagnosis.explanation = (
            "You may be confusing this with a similar concept. Let's compare them side-by-side."
        )
        diagnosis.remediation_params = {
        ...
        diagnosis.explanation = (
            "Normal memory decay. Continued spaced repetition will strengthen this memory trace."
        )
...
class VelocityMetrics:
    ...
    estimated_time_to_mastery: float | None = None  # hours
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "estimated_time_to_mastery_hours": (
                round(self.estimated_time_to_mastery, 1) if self.estimated_time_to_mastery else None
            ),
        }
...
def compute_velocity(
    ...
    correct / atoms_completed if atoms_completed > 0 else 0
    ...
    if len(session_history) >= 6:
        ...
        second_accuracy = sum(1 for h in second_half if h.get("is_correct", False)) / len(
            second_half
        )
